...
@app.get("/api/health")
def health() -> dict:
    from .openrouter import key_status

    return {"ok": True, "model": settings.openrouter_model, "transcriber": settings.transcriber,
            "transcribe_model": settings.openrouter_transcribe_model,
            "key_length": len(settings.openrouter_key),
            "openrouter": key_status(),
